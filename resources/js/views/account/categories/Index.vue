<template>
    <h1 class="text-3xl mb-3">Categories</h1>
    <div class="grid mb-3 justify-end">
        <router-link :to="{ name: 'account.categories.create' }" class="btn-primary">Add category</router-link>
    </div>
            ...
            <table class="w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th class="px-6 py-3">
                            <div class="flex flex-row items-center cursor-pointer" @click="updateOrdering('name')">
                                <div class="tracking-wider" :class="{ 'font-bold text-blue-600': orderColumn === 'name' }">
                                    Name
                                </div>
                                <div class="select-none">
                                    <span :class="{
                ...
                <tbody>
                    <tr v-for="category in categories.data" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link v-if="true" :to="{ name: 'account.categories.edit', params: { id: category.id } }" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</router-link>
                            <a href="#" v-if="true" @click.prevent="deleteCategory(category.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline ml-2">Delete</a>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
        <TailwindPagination :data="categories" @pagination-change-page="getResults" class="mt-4" />
    </div>
</template>
...
<script setup>
    ...
    const { categories, getCategories, deleteCategory, isLoading } = useCategories();
