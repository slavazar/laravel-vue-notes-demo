<template>
    <h1 class="text-3xl mb-3">Category</h1>
    <div class="underline mb-3">
        <router-link :to="{ name: 'account.categories.index' }">return</router-link>
    </div>
    <div class="relative overflow-x-auto p-6 bg-white">
        ...
        <form @submit.prevent="updateCategory(category)" class="w-full lg:w-3/4">
            <!-- Title -->
            <div>
                <label for="category-name" class="block font-medium text-sm text-gray-700">
                    Name
                </label>
                <input v-model="category.name" id="category-name" type="text" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
...
const { category, getCategory, updateCategory, actionErrors, validationErrors, isLoading } = useCategories();
